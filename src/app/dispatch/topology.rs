...
impl App {
    pub(in crate::app) fn check_remote_workspace_request(
        &self,
        method: &str,
        p: &Value,
...
impl App {
    pub(in crate::app) fn require_local_workspace(
        &self,
        index: usize,
    ) -> Result<(), (String, String)> {
        let workspace = self
...
mod tests {
    ...
    #[test]
    fn auto_split_prefers_workspace_owner_pixels_over_foreground_pixels() {
        let _env = crate::persist::test_env("pane-split-owner-cell-aspect");
        let (tx, _rx) = std::sync::mpsc::channel();
        let mut app = App::new(80, 24, tx).unwrap();
        app.last_pane_area = Rect::new(0, 0, 60, 40);
        app.set_client_cell_pixels(10, 10);
        app.workspaces[app.active_ws].cell_pixels = Some((8, 24));
        app.dispatch("pane.split", &json!({"direction":"auto"}))
            .unwrap();
        assert_eq!(
            split_axis(&app),
            1,
            "owner's tall cells must override foreground square cells"
        );
    }
