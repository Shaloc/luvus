...
impl App {
    ...
    pub(crate) fn pane_title(&self, pane: PaneId) -> Option<String> {
        ...
        self.module_pane_title(pane)
    }

    /// Bounded metadata snapshots must never acquire a terminal engine lock.
    pub(crate) fn cached_pane_title(&self, pane: PaneId) -> Option<String> {
        self.status
            .get(&pane)
            .and_then(|status| status.detected_title.as_deref())
            .map(strip_title_icon)
            .filter(|title| !title.is_empty())
            .or_else(|| self.module_pane_title(pane))
    }

    fn module_pane_title(&self, pane: PaneId) -> Option<String> {
        if let Some(title) = self
            .agent_title_panes
            .get(&pane)
