...
impl App {
    ...
    fn focus_next_agent_in_state(
        ...
        let mut matches: Vec<super::AgentTarget> = Vec::new();
            ...
            for tab in &ws.tabs {
                for id in tab.layout.leaves() {
                    if let Some(s) = self.status.get(&id) {
                        ...
                        if is_agent && s.state == state {
                            matches.push(super::AgentTarget::Live(id));
                        }
                    }
                }
            }
        ...
        let next = match matches.iter().position(|b| *b == focused) {
