...
static HELP: &[Translation] = &[
    tr!(
        ...
        "redémarrer toutes les sessions actives sur cet hôte",
        "alle laufenden Sitzungen auf diesem Host neu starten",
        "mulai ulang semua sesi yang berjalan di host ini",
        "重启本机所有运行中的会话",
        "このホストで実行中の全セッションを再起動",
        "이 호스트에서 실행 중인 모든 세션 다시 시작"
    ),
    tr!(
        "toggle global same-name session merging",
        "alternar la combinación global de sesiones homónimas",
        "alternar a mesclagem global de sessões homônimas",
        "activer la fusion globale des sessions homonymes",
        "globales Zusammenführen gleichnamiger Sitzungen umschalten",
        "aktifkan penggabungan global sesi bernama sama",
        "全局开关：自动合并同名会话",
        "同名セッションの全体統合を切り替え",
        "같은 이름 세션의 전역 병합 전환"
    ),
    ...
    tr!(
        ...
        "Controlar comandos respaldados por el servidor en un host SSH configurado",
        "Controlar comandos do servidor em um host SSH configurado",
        "Contrôler les commandes gérées par le serveur sur un hôte SSH configuré",
        "Servergestützte Befehle auf einem konfigurierten SSH-Host steuern",
        "Kendalikan perintah berbasis server pada host SSH terkonfigurasi",
        "控制已配置 SSH 主机上的服务端命令",
        "設定済み SSH ホスト上のサーバー管理コマンドを操作",
        "설정된 SSH 호스트의 서버 기반 명령 제어"
    ),
    tr!(
        "Control the remote session through SSH",
        "Controlar la sesión remota mediante SSH",
        "Controlar a sessão remota por SSH",
        "Contrôler la session distante via SSH",
        "Remote-Sitzung über SSH steuern",
        "Kendalikan sesi jarak jauh melalui SSH",
        "通过 SSH 控制远端会话",
        "SSH 経由でリモートセッションを操作",
        "SSH를 통해 원격 세션 제어"
    ),
    tr!(
        "Target a literal Host from ~/.ssh/config",
        "Usar un Host literal de ~/.ssh/config",
        "Usar um Host literal de ~/.ssh/config",
        "Cibler un Host littéral de ~/.ssh/config",
        "Einen wörtlichen Host aus ~/.ssh/config auswählen",
        "Targetkan Host literal dari ~/.ssh/config",
        "指定 ~/.ssh/config 中明确声明的 Host",
        "~/.ssh/config の明示的な Host を対象にする",
        "~/.ssh/config의 명시적 Host 지정"
    ),
    tr!(
        "route the TUI or server-backed command to an SSH-config host",
        "dirigir la TUI o el comando respaldado por el servidor a un host de la configuración SSH",
        "direcionar a TUI ou o comando do servidor a um host da configuração SSH",
        "acheminer la TUI ou la commande gérée par le serveur vers un hôte de la configuration SSH",
        "TUI oder servergestützten Befehl an einen SSH-Konfigurationshost leiten",
        "arahkan TUI atau perintah berbasis server ke host konfigurasi SSH",
        "将 TUI 或服务端命令路由到 SSH 配置中的主机",
        "TUI またはサーバー管理コマンドを SSH 設定のホストへ送る",
        "TUI 또는 서버 기반 명령을 SSH 설정 호스트로 전달"
    ),
    tr!(
        "Manage local, remote, and merged server sessions",
        "Gestionar sesiones de servidor locales, remotas y combinadas",
        "Gerenciar sessões de servidor locais, remotas e mescladas",
        "Gérer les sessions serveur locales, distantes et fusionnées",
        "Lokale, Remote- und zusammengeführte Serversitzungen verwalten",
        "Kelola sesi server lokal, jarak jauh, dan gabungan",
        "管理本地、远端和合并显示的服务器会话",
        "ローカル、リモート、マージ済みサーバーセッションを管理",
        "로컬, 원격 및 병합된 서버 세션 관리"
    ),
    tr!(
        "list local and discovered remote sessions",
        "listar sesiones locales y remotas descubiertas",
        "listar sessões locais e remotas descobertas",
        "lister les sessions locales et distantes découvertes",
        "lokale und gefundene Remote-Sitzungen auflisten",
        "daftar sesi lokal dan jarak jauh yang ditemukan",
        "列出本地会话和发现的远端会话",
        "ローカルおよび検出されたリモートセッションを一覧表示",
        "로컬 및 검색된 원격 세션 나열"
    ),
    tr!(
        "start an SSH-config remote after exact-version validation",
        "iniciar una sesión remota SSH tras validar la versión exacta",
        "iniciar uma sessão remota SSH após validar a versão exata",
        "démarrer une session distante SSH après validation exacte de la version",
        "SSH-Remote-Sitzung nach exakter Versionsprüfung starten",
        "mulai sesi jarak jauh SSH setelah validasi versi tepat",
        "精确校验版本后启动 SSH 配置中的远端会话",
        "完全一致するバージョンを確認して SSH リモートセッションを起動",
        "정확한 버전 확인 후 SSH 원격 세션 시작"
    ),
    tr!(
        "forget a registered remote session",
        "olvidar una sesión remota registrada",
        "esquecer uma sessão remota registrada",
        "oublier une session distante enregistrée",
        "eine registrierte Remote-Sitzung vergessen",
        "lupakan sesi jarak jauh terdaftar",
        "移除已注册的远端会话",
        "登録済みリモートセッションを削除",
        "등록된 원격 세션 제거"
    ),
    tr!(
        "discover sessions on selected SSH hosts",
        "descubrir sesiones en los hosts SSH seleccionados",
        "descobrir sessões nos hosts SSH selecionados",
        "découvrir les sessions des hôtes SSH sélectionnés",
        "Sitzungen auf ausgewählten SSH-Hosts finden",
        "temukan sesi pada host SSH yang dipilih",
        "发现已选 SSH 主机上的会话",
        "選択した SSH ホストのセッションを検出",
        "선택한 SSH 호스트의 세션 검색"
    ),
    tr!(
        "show local and discovered same-name remotes together",
        "mostrar juntos la sesión local y las remotas descubiertas con el mismo nombre",
        "mostrar juntos a sessão local e as remotas descobertas com o mesmo nome",
        "afficher ensemble les sessions locales et distantes découvertes du même nom",
        "lokale und gefundene gleichnamige Remote-Sitzungen zusammen anzeigen",
        "tampilkan sesi lokal dan jarak jauh bernama sama yang ditemukan bersama-sama",
        "同时显示本地会话和发现的同名远端会话",
        "ローカルと検出された同名リモートをまとめて表示",
        "로컬 및 검색된 동명 원격 세션 함께 표시"
    ),
    tr!(
        "run server-backed CLI controls against the selected",
        "ejecutar controles CLI respaldados por el servidor contra la sesión seleccionada",
        "executar controles CLI do servidor na sessão selecionada",
        "exécuter les commandes CLI gérées par le serveur sur la session sélectionnée",
        "servergestützte CLI-Befehle für die ausgewählte Sitzung ausführen",
        "jalankan kontrol CLI berbasis server pada sesi yang dipilih",
        "对所选会话执行服务端 CLI 控制命令",
        "選択したセッションに対してサーバー管理 CLI 操作を実行",
        "선택한 세션에 서버 기반 CLI 제어 실행"
    ),
    tr!(
        "session on a literal Host from ~/.ssh/config",
        "en un Host literal de ~/.ssh/config",
        "em um Host literal de ~/.ssh/config",
        "sur un Host littéral de ~/.ssh/config",
        "auf einem wörtlichen Host aus ~/.ssh/config",
        "pada Host literal dari ~/.ssh/config",
        "目标主机必须是 ~/.ssh/config 中明确声明的 Host",
        "~/.ssh/config の明示的な Host 上のセッション",
        "~/.ssh/config의 명시적 Host에 있는 세션"
    ),
    tr!(
        "(put --host before prompt/pass-through commands)",
        "(coloca --host antes de comandos con texto o argumentos reenviados)",
        "(coloque --host antes de comandos com texto ou argumentos repassados)",
        "(placez --host avant les commandes de texte ou de transmission)",
        "(--host vor Prompt- und Durchreichbefehle setzen)",
        "(letakkan --host sebelum perintah prompt/penerusan)",
        "（对提示词或透传命令，请把 --host 放在命令前）",
        "（プロンプト／パススルーコマンドでは --host を前に置く）",
        "(프롬프트/전달 명령 앞에 --host 배치)"
    ),
    tr!(
...
static TEXT: &[Translation] = &[
    ...
    tr!("restarted", "reiniciado", "reiniciado", "redémarré", "neu gestartet", "dimulai ulang", "已重启", "再起動済み",
        "재시작됨"),
    tr!("{count} sessions failed to restart", "No se pudieron reiniciar {count} sesiones", "Falha ao reiniciar {count} sessões", "Échec du redémarrage de {count} sessions", "{count} Sitzungen konnten nicht neu gestartet werden", "{count} sesi gagal dimulai ulang", "{count} 个会话重启失败", "{count} 個のセッションを再起動できませんでした", "{count}개 세션을 다시 시작하지 못했습니다"),
    tr!("stopped", "detenida", "parada", "arrêtée", "gestoppt", "berhenti", "已停止", "停止済み",
        "중지됨"),
    tr!("remote", "remota", "remota", "distante", "remote", "jarak jauh", "远端", "リモート",
        "원격"),
    tr!("remote merged", "remota combinada", "remota mesclada", "distante fusionnée", "remote zusammengeführt", "jarak jauh gabungan", "远端已合并显示", "リモート・マージ", "원격 병합"),
    tr!("merged", "combinada", "mesclada", "fusionnée", "zusammengeführt", "gabungan", "已合并显示", "マージ済み", "병합됨"),
    tr!("standalone", "independiente", "independente", "autonome", "eigenständig", "mandiri", "独立", "単独", "독립"),
    tr!("registered remote session", "sesión remota registrada", "sessão remota registrada", "session distante enregistrée", "Remote-Sitzung registriert", "sesi jarak jauh terdaftar", "已注册远端会话", "リモートセッションを登録しました", "원격 세션 등록됨"),
    tr!("removed remote session", "sesión remota eliminada", "sessão remota removida", "session distante supprimée", "Remote-Sitzung entfernt", "sesi jarak jauh dihapus", "已移除远端会话", "リモートセッションを削除しました", "원격 세션 제거됨"),
    tr!("session merge", "combinación de sesión", "mesclagem de sessão", "fusion de session", "Sitzungszusammenführung", "penggabungan sesi", "会话合并显示", "セッションのマージ", "세션 병합"),
    tr!("on", "activado", "ativado", "activé", "an", "aktif", "开启", "オン", "켜짐"),
    tr!("off", "desactivado", "desativado", "désactivé", "aus", "nonaktif", "关闭", "オフ", "꺼짐"),
