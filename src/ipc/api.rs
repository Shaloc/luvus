...
fn control_action_response(
    ...
    let staged_image = if action == "paste_image" {
        ...
        let path = match crate::terminal::clipboard::stage_web_png(encoded) {
            Ok(path) => path,
            Err(_) => {
                return json!({"id":id,"error":{"code":"invalid_params",
    ...
    {
        crate::terminal::clipboard::discard_staged_png(staged_image.as_deref().unwrap());
    }
...
mod tests {
    ...
    fn terminal_control_frames_reuse_strict_uhp_actions() {
        ...
        let png =
            crate::terminal::clipboard::png::encode_rgba_png(1, 1, |_, _| [1, 2, 3, 255]).unwrap();
