...
impl Completion {
    ...
    /// Reuse the display owner's bounded event channel. The callback must not block.
    pub(crate) fn with_wake(wake: impl Fn() + Send + Sync + 'static) -> Arc<Self> {
        Arc::new(Self {
            state: Mutex::new((true, None)),
...
mod tests {
    ...
    fn clipboard_retired_completion_rejects_late_results() {
        let completion = super::Completion::with_wake(|| {});
