...
const homePrefix = path.join(repoRoot, "target", "luvus-web-integration-");
...
const commonEnv = { ...Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith("LUVUS_"))), LUVUS_HOME: home };
delete commonEnv.LUVUS_SOCKET_PATH;
delete commonEnv.LUVUS_SESSION;

let bridge;
try {
  // The fixture lives under target/, ignored by the parent checkout. Give file
  // search its own repository boundary so Cargo.fixture remains discoverable.
  command("git", ["init", "--quiet"], commonEnv);
