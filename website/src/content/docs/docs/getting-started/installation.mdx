...
luvus ships as a **single binary** with no runtime dependencies.
...
<Tabs syncKey="install">
  ...
  <TabItem label="Debian / Fedora">
    ...
    (`git`, `openssh-client`, `xclip`) are **Recommends**, while
    `wl-clipboard` is suggested for Wayland clipboard copy and image paste. A
    desktop can install the applicable helper while a minimal server can skip
    both without breaking Luvus. Good for provisioning a Linux server.
  </TabItem>
  <TabItem label="Nix">
