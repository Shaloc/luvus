...
`luvus server restart` restarts only the selected session. Add `--all`
to restart every **running** session in the selected
`LUVUS_HOME` on this machine, including `default`. Saved but stopped sessions
stay stopped. Each session uses the normal stop/start and snapshot-restore
path; existing pane processes end, and only supported native agent sessions
can resume. Run this from a terminal outside the sessions being restarted.

```sh
luvus server restart --all
luvus server restart --all --json
luvus --host dev-207 server restart --all
```

`--host` applies the batch on that one enabled SSH host, never on the local
machine as well. Without `--host`, selected SSH hosts and remote-owned panes
are not restarted; local presentation servers are local sessions. An explicit
`--session` does not narrow an `--all` batch. Individual failures are reported
and do not prevent attempts on the remaining running sessions; any failure
makes the command exit nonzero. JSON output includes `scope`, `sessions`,
`restarted`, `failed`, and the names of `skipped` stopped sessions.
...
See the [Luvus Web guide](/docs/guides/web/) for first-time pairing, phone
access through a private TLS provider, multiple devices, terminal input, file
uploads, session switching, and troubleshooting.
...
### Restart one terminal pane

Click **↻** at the top right of the focused pane, then confirm, or run
`luvus pane restart <id>`. The CLI acts immediately. Restart hangs up that
pane’s current process using the same lifecycle as closing a terminal. Its
layout slot, name, pin, and working directory are preserved; other panes and
the server keep running. Deliberately detached/nohup jobs follow the usual
terminal-close behavior.

A trusted, supported native agent session resumes using the existing launch-flag
setting. Otherwise a fresh shell starts; arbitrary shell commands and terminal
scrollback are not replayed. The replacement receives a new pane ID. Module panes
must be reopened through their module; native file and diff views have no PTY to
restart. The button is shown in desktop pane headers when space permits.

For a remote pane, use the button in its remote frame or address its owner with
`luvus --host <host> --session <name> pane restart <id>`. Install the updated
binary and load it into the owning server once before using this control.
