...
| opencode | ✓ | ✓ | no | ✓ |
| Kimi Code CLI | ✓ | ✓ | no | ✓ |
| Qoder CLI | ✓ | ✓ with integration | no | session only |
| Grok Build | ✓ | ✓ | ✓ | ✓ |
...
- **Hook**, via `luvus integration install <agent>`, makes the agent report its
  ...
  Qoder CLI uses screen evidence for live state, so detection works without an
  integration. `luvus integration install qodercli` adds one Luvus-owned
  `SessionStart` command under `~/.qoder/settings.json` (or
  `$QODER_CONFIG_DIR/settings.json`) and records only the exact session id.
  Luvus then restores that session with `qodercli --resume <id>`. The Qoder
  config directory must already exist; install and uninstall preserve every
  unrelated setting and hook.
