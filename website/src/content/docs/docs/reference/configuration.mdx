...
| `remote-sessions.json` | SSH remote session registrations and the global same-name merge preference |
...
| `layout.workspace_display` | `flat` (default) keeps the workspace list with hostname tags; `tree` groups workspaces and their worktrees under collapsible machine headings. Change it live in Settings → Layout → Workspace display. It does not change merge, SSH admission, or workspace indices. |
| `layout.auto_workspace_rehome` | `false` by default, including existing configurations without this field. Settings → Layout → Auto-move tabs by directory opts into creating workspaces and moving whole tabs based on scanned process directories. When off, terminal directory and workspace branch tracking continue, but scans cannot create workspaces, move tabs, or close emptied source workspaces. Manual operations are unchanged. This is an owner-server setting; configure remote owners separately. |
...
| `remote_hosts` | SSH config aliases enabled in Settings → Remote. Defaults to `[]`. Selecting connects and discovers sessions; deselecting disconnects without stopping remote panes. Opening or refreshing the session switcher discovers new remote sessions. Merge includes discovered same-name sessions. |
...
| `bars.top_right` / `bars.bottom_right` / `bars.off` | canonical widget ids (`module:id`) in each Luvus Bar placement. `core:runtime-status` defaults to Bottom. `core:focused-pane` (current tab, pane ID, agent type and state) defaults to Off. Live content and notifications are never persisted. |
