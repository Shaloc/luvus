...
Kimi, Qoder CLI, Grok, Hermes CLI, Letta Code, Muse Code, Arc Studio CLI, Pi, Oh My Pi (OMP),
...
The workspace status dot combines all panes across its tabs, including remote
workspaces, in this priority order: **blocked > working > done > idle**. A finished
agent does not hide another agent that is still working.
...
Codex's idle composer can contain decorative braille dots. Rows made entirely
of those dots and whitespace do not count as screen spinners; spinners with
activity text and interrupt hints still indicate work. Remote agent rows also
refresh when an agent appears or exits without changing its activity state.
...
Aider, OpenCode V2, Kilo Code, Kiro, Qoder CLI, Cursor, Amp, Droid, Muse, Devin, Letta Code, Arc Studio CLI, and
...
| Qoder CLI | its exact session id from the optional integration |
...
The Codex integration writes `hooks.json` and `luvus-agent-hook.sh` under
`$CODEX_HOME`, or `~/.codex` when it is unset. These hooks report session identity,
not an activity-state lease: `agent explain` can show `authority: null` while
the integration is installed and a session is bound. Codex activity still uses
native screen detection. Installed verifies the expected hook registration,
script contents and executable permissions, plus the Unix launcher. It does not
prove that Codex enabled, trusted, or executed a hook.

On macOS and Linux, the integration also installs a private `codex` launcher at
`$LUVUS_HOME/integrations/bin/codex` (normally `~/.luvus/integrations/bin/codex`,
or `~/.luvus-dev/integrations/bin/codex` for debug builds). New panes prepend
that directory to PATH. The launcher passes the current pane's route as
session-scoped hook configuration, so a shared Codex app-server can report to
the right pane even when its daemon environment is missing or stale. The
registered global hook does nothing without this explicit routing context.

After upgrading, run `luvus integration install codex` and open a new pane.
Check `command -v codex`: shell startup files, aliases, or an absolute binary
path can bypass the private launcher. You can invoke the launcher by its full
path if needed. Both the launcher and hook require Python 3. Administrative
commands pass through unchanged. Explicit Codex `--remote` connections do not
receive a local pane route.

Launch Codex normally, then choose **Review hooks** at its **Hooks need review**
prompt, or open `/hooks`. Review the `SessionStart` and `UserPromptSubmit` entries
whose source is **Session flags** and whose command invokes the Luvus hook with
`LUVUS_CODEX_HOOK_CONTEXT=1`. Trust those individual definitions with Codex's
review UI. Approval applies to the exact hook hash, so a changed pane, socket,
binary path, or hook definition can require fresh review. Submit a prompt after
approval if the initial session-start hook was skipped.

The launcher does not grant trust, edit Codex's trust records, or add a bypass
flag. If you explicitly use `codex --dangerously-bypass-hook-trust`, Codex skips
trust checks for all enabled hooks in that invocation, not only Luvus's hooks.
That flag remains optional; the native review flow works for the launcher's
startup configuration. See the [Codex hooks documentation](https://learn.chatgpt.com/docs/hooks).
...
```sh
luvus integration install claude    # or: copilot · codex · antigravity · letta · opencode · kimi · qodercli · grok · hermes · omp
```
...
- **qodercli**: adds one Luvus-owned `SessionStart` command to
  `~/.qoder/settings.json`, or the configuration selected by
  `QODER_CONFIG_DIR`. It reports only the exact session id used by
  `qodercli --resume <id>`; screen detection remains responsible for state.
  The Qoder config directory must already exist, and unrelated settings and
  hooks are preserved.
