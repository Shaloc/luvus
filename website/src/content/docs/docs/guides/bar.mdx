...
small actions in the existing local tab and status rows.
...
For managed remote workspaces, **Top** uses a display-side navigation row above
the owner's tabs. This reserves one row from the remote viewport and resizes
its PTY accordingly, so local widgets never cover remote tabs or input cells.
**Bottom** continues to use the existing status row.
...
The optional **Current tab / pane** widget (`core:focused-pane`) starts **Off**.
Select **Bottom** in Settings to see, for example,
`tab 2 · pane 27 · ● qodercli working`. The tab position is 1-based and the
pane ID is the owner's opaque identifier. Local, remote and merged workspaces
use their actual focused pane; shells and native views omit the agent label.
Disconnected or not-yet-aligned remote metadata is hidden. In narrow rows it
uses compact labels such as `t2 p27`, then the standard overflow if needed.
Mode-specific key guidance retains priority over bottom widgets.
